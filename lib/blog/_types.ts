 ...
 * `lib/blog-posts-config.ts`. The dynamic route at `app/blog/[slug]`
 * resolves any registered slug.
 */
...
export type CalloutBlock = {
  ...
  linkText: string
  /** Internal path, e.g. "/plumbing/clogged-drain" */
  href: string
}

/** Outbound resource link (EPA, DOE, NFPA, etc.) — flagged so we can render
...
export type BlogPost = {
  ...
  /** Hero image path under /public, e.g. "/blog/sewer-line-repair-fort-worth.webp" */
  heroImage: string
   ...
   * points elsewhere instead of `${siteConfig.url}/blog/${slug}`. Used to
