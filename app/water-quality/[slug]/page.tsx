...
export async function generateStaticParams() {
  return waterQualityCategoryServices.map((cfg) => ({ slug: cfg.slug }))
}
...
export async function generateMetadata({
  ...
  if (!cfg || !cfg.href.startsWith("/water-quality/")) return {}
...
export default async function WaterQualityServicePage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  const { slug } = await params
  const cfg = getServiceConfig(slug)
  if (!cfg || !cfg.href.startsWith("/water-quality/")) notFound()
