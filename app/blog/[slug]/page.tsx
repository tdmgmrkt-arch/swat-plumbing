...
export async function generateMetadata({
  ...
  // Allow a post to override its canonical when it competes with a service
  // page for the same transactional intent (resolves SXO cannibalization).
  const canonical =
    post.canonicalOverride ?? canonicalUrl(`/blog/${post.slug}`)
  const ogImage = `${siteConfig.url}${post.heroImage}`

  return {
    // Layout's title.template appends "| S.W.A.T. Plumbing LLC" — don't add
    // another "S.W.A.T. Plumbing Blog" suffix here or the title double-stacks.
    title: post.title,
...
export default async function BlogPostPage({
  ...
  const dispatchPhone =
    post.city === "Fort Worth"
      ? siteConfig.phone.fortWorth
      : siteConfig.phone.aledo
  const dispatchTel =
    post.city === "Fort Worth"
      ? siteConfig.phone.fw_tel
      : siteConfig.phone.aled_tel
  ...
  return (
    <>
      ...
      <main id="main-content">
                ...
                <div className="mt-12 relative bg-[#0a0c0e] border border-white/12 rounded-sm overflow-hidden">
                  ...
                  <div className="p-6 lg:p-8 pl-7 lg:pl-9">
                    ...
                    <h2 className="text-xl sm:text-2xl font-black text-white tracking-tight leading-tight mb-3">
                      Ready to talk to a licensed plumber?
                    </h2>
        ...
        {related.length > 0 && (
          <section
            className="relative bg-[#080a0c] border-t border-white/8 py-16 lg:py-20 overflow-hidden"
...
function BlockRenderer({ block }: { block: ContentBlock }) {
  ...
  if (block.type === "callout") {
    return (
      <aside
        className="relative bg-[#0a0c0e] border border-white/10 rounded-sm overflow-hidden my-2"
  ...
  if (block.type === "outbound") {
    return (
      <aside
        className="relative bg-[#0a0c0e] border border-dashed border-white/15 rounded-sm overflow-hidden my-2"
...
function RelatedCard({ post }: { post: BlogPost }) {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group block relative bg-[#0e1012] border border-white/10 hover:border-red-600/40 rounded-sm overflow-hidden transition-colors h-full flex-col flex"
    >
      <div className="relative aspect-[16/9] bg-[#0a0c0e] overflow-hidden">
        ...
        />
        <div
          className="absolute inset-0 bg-linear-to-t from-[#0e1012]/80 via-transparent to-transparent"
          aria-hidden="true"
        />
