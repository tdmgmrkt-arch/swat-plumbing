...
export async function generateStaticParams() {
  return waterHeaterCategoryServices.map((cfg) => ({ slug: cfg.slug }))
}
...
export async function generateMetadata({
  ...
  if (!cfg || !cfg.href.startsWith("/water-heater/")) return {}
...
export default async function WaterHeaterServicePage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  const { slug } = await params
  const cfg = getServiceConfig(slug)
  if (!cfg || !cfg.href.startsWith("/water-heater/")) notFound()
