...
> = {
  "HQ in Aledo": MapPin,
  "On Camp Bowie": MapPin,
  "<25-Min from Hub": Zap,
  "24/7 Dispatch": Clock,
  "24/7 FW Dispatch": Clock,
  "Texas Master Plumber": ShieldCheck,
  "TX Master Plumber": ShieldCheck,
  "Upfront Pricing": BadgeCheck,
  "Flat-Rate Pricing": BadgeCheck,
...
/** Returns the tel href and display number for a hub slug */
function hubPhone(slug: "aledo" | "fort-worth"): { tel: string; display: string } {
  if (slug === "aledo") {
    return { tel: siteConfig.phone.aled_tel, display: siteConfig.phone.aledo }
  }
  return { tel: siteConfig.phone.fw_tel, display: siteConfig.phone.fortWorth }
}

export default function CityHero({ cfg }: { cfg: CityConfig }) {
  const primary = hubPhone(cfg.primaryHubSlug)
  const secondary = hubPhone(cfg.primaryHubSlug === "aledo" ? "fort-worth" : "aledo")

  return (
    <section
      className="relative isolate bg-[#080a0c] py-20 lg:py-28 overflow-hidden"
      aria-labelledby="city-hero-heading"
    >
      {/* Neighborhood backdrop — atmospheric on left, visible on right */}
      <Image
        src="/hero-images/city-hero.webp"
        alt=""
        fill
        sizes="100vw"
        className="object-cover object-center -z-10 opacity-[0.28]"
        priority
      />

      {/* Stacked overlays — solid behind copy, transparent behind right column */}
      <div
        className="absolute inset-0 -z-10 bg-linear-to-r from-[#050608] from-0% via-[#080a0c]/85 via-45% to-[#080a0c]/35 pointer-events-none"
        aria-hidden="true"
      />
      <div
        ...
        <div className="max-w-3xl">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-10">
            ...
            <Link
              ...
              aria-label={`Call ${cfg.name} at ${primary.display}`}
            >
            ...
            <Link
              ...
              className={cn(
                "inline-flex items-center justify-center gap-2",
                "border border-white/15 text-white/70 bg-transparent hover:bg-white/5 hover:border-white/28",
                "font-medium text-sm px-6 py-4 rounded-sm min-h-[52px]",
                "transition-colors tracking-wide uppercase"
              )}
              aria-label={`Call alternate hub at ${secondary.display}`}
            >
