...
export default function CityNearbyCI({ cfg, nearby }: Props) {
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-6">
        <div className="mb-10 max-w-2xl">
          ...
          <p className="mt-4 text-white/50 text-sm leading-relaxed">
            S.W.A.T. Plumbing covers {cfg.name} and the surrounding area with the same
            crew, same pricing, and same 24/7 dispatch.
          </p>
        </div>
        ...
        >
          {nearby.map((nc) => {
            ...
            const baseClass = cn(
              "group relative block bg-[#0a0c0e] border rounded-sm p-5 transition-colors w-full",
              nc.isBuilt
              ...
              return (
                <li key={nc.slug}>
                  <Link
                    ...
                    className={baseClass}
                    aria-label={`Plumbing in ${nc.name}, TX`}
                  >
                    {cardContent}
                  </Link>
                </li>
