...
/**
 * Dark-mode card used in every category hub's service grid.
 * Three lines from the service's scope bullets fill the card with substance
 * without leaning on the longer heroPainLine. Primary cards (emergency, hub
 * anchors) get a red command stripe and pulse badge.
 */
export default function HubServiceCard({
  ...
  const previewBullets = cfg.scopeBullets.slice(0, 3)

  return (
    <Link
      href={cfg.href}
      aria-label={`${cfg.name} — view service`}
      className={cn(
        ...
        primary
          ...
          : "bg-[#0e1012] border-white/10 hover:border-red-600/40 hover:bg-[#13181c]"
      )}
    >
      ...
      <div className="relative flex flex-col h-full">
        ...
        {/* Short hook from heroPainLine, trimmed for card density */}
        <p className="text-white/55 text-sm leading-relaxed mb-5 line-clamp-2">
          {cfg.heroPainLine}
        </p>

        {/* Three scope bullets as substance */}
