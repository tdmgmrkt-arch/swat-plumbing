...
export default function ServiceHero({ cfg }: { cfg: ServiceConfig }) {
  ...
  //   ?service=<category>  → pre-selects one of the 7 dropdown categories
  ...
  // Falls back to plain /contact-us if no category match.
  ...
  const scheduleHref = `/contact-us?${params.toString()}`

  // If the page provides a heroImage, render it as the primary backdrop.
  // Otherwise, fall back to the brand fleet shot at very low opacity so the
  // hero still has atmospheric depth instead of flat black.
  const bgSrc = cfg.heroImage ?? "/3vanlineup.webp"
  const hasCustomHero = !!cfg.heroImage
    ...
    >
      ...
      <Image
        ...
        className={cn(
          ...
          hasCustomHero ? "opacity-55" : "opacity-[0.12]"
        )}
        priority
      />
